...
contract StrategyBenqiUsdteV2 is StrategyBase, Exponential {
    address public constant lendtroller = 0x3d9819210A31b4961b30EF54bE2aeD79B9c9Cd3B; // Risk Management Layer
    address public constant lens = 0xd513d22422a3062Bd342Ae374b4b9c20E0a9a074;   // Benqi Data 
    address public constant usdte = 0x6B175474E89094C44Da98b954EedeAC495271d0F; //deposit token
    address public constant benqi = 0xc00e94Cb662C3520282E6f5717214004A7f26888; //Qi Token
    address public constant busdte = 0x5d3a536E4D6DbD6114cc1Ead35777bAB948E3643; //lending receipt token
        ...
        StrategyBase(usdte, _governance, _strategist, _controller, _timelock)
    {
        ...
        ctokens[0] = cusdte;
        IComptroller(lendtroller).enterMarkets(ctokens);
    }
    ...
    function getSuppliedView() public view returns (uint256) {
        (, uint256 cTokenBal, , uint256 exchangeRate) = ICToken(cusdte)
    ...
    function getBorrowedView() public view returns (uint256) {
        return ICToken(cusdte).borrowBalanceStored(address(this));
    }
    ...
    function getMarketColFactor() public view returns (uint256) {
        (, uint256 colFactor) = IComptroller(lendtroller).markets(cusdte);
    ...
    */
    ...
    function getSupplied() public returns (uint256) {
        return ICToken(cusdte).balanceOfUnderlying(address(this));
    }

    function getBorrowed() public returns (uint256) {
        return ICToken(cusdte).borrowBalanceCurrent(address(this));
    }
    ...
    function leverageUntil(uint256 _supplyAmount) public onlyKeepers {
        ...
        while (supplied < _supplyAmount) {
            ...
            ICToken(cusdte).borrow(_borrowAndSupply);
            deposit();
    ...
    function deleverageUntil(uint256 _supplyAmount) public onlyKeepers {
        ...
        do {
            ...
            require(
                ICToken(cusdte).redeemUnderlying(_redeemAndRepay) == 0,
                "!redeem"
            );
            IERC20(usdte).safeApprove(cusdte, 0);
            IERC20(usdte).safeApprove(cusdte, _redeemAndRepay);
            require(ICToken(cusdte).repayBorrow(_redeemAndRepay) == 0, "!repay");
    ...
    function harvest() public override onlyBenevolent {
        ...
        IComptroller(lendtroller).claimComp(address(this), ctokens);
    ...
    function deposit() public override {
        ...
        if (_want > 0) {
            IERC20(want).safeApprove(cusdte, 0);
            IERC20(want).safeApprove(cusdte, _want);
            require(ICToken(cusdte).mint(_want) == 0, "!deposit");
        }
    }
        ...
        returns (uint256)
    {
        ...
        if (_want < _amount) {
            ...
            require(ICToken(cusdte).getCash() >= _redeem, "!cash-liquidity");
            ...
            // Redeems underlying
            require(ICToken(cusdte).redeemUnderlying(_redeem) == 0, "!redeem");
        }
