...
// File contracts/strategies/traderJoe/strategy-joe-avax-fly.sol
...
contract StrategyJoeAvaxFly is StrategyJoeRushFarmBase {
    ...
    function harvest() public override onlyBenevolent {
        ...
        uint256 _fly = IERC20(fly).balanceOf(address(this));            // get the balance of the FLY tokens
        if (_fly > 0){
            IERC20(fly).safeApprove(joeRouter, 0);
            IERC20(fly).safeApprove(joeRouter, _fly);
            _swapTraderJoe(fly, wavax, _fly); 
        }

        uint256 _joe = IERC20(joe).balanceOf(address(this));            // get the balance of the JOE tokens
        if (_joe > 0){
            IERC20(joe).safeApprove(joeRouter, 0);
        ...
        uint256 _wavax = IERC20(wavax).balanceOf(address(this));        // get the balance of the WAVAX tokens
        if (_wavax > 0){
