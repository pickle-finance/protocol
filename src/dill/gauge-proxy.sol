...
contract GaugeV2 is ReentrancyGuard {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    ...
    function derivedBalance(address account) public view returns (uint256) {
        ...
        uint256 _adjusted = (_totalSupply.mul(DILL.balanceOf(account)).div(DILL.totalSupply())).mul(60).div(100);
        uint256 dillBoostedDerivedBal = Math.min(_derived.add(_adjusted), _balance);
        ...
        for (uint256 i = 0; i < lockedStakes[account].length; i++) {
            ...
            lockBoostedDerivedBal = lockBoostedDerivedBal.add(combined_boosted_amount);
        }

        return dillBoostedDerivedBal.add(lockBoostedDerivedBal);
    }
    ...
    function earned(address account) public view returns (uint256) {
        return derivedBalances[account]
    ...
    function depositAll(uint256 secs) external {
        _deposit(TOKEN.balanceOf(msg.sender), msg.sender, secs, block.timestamp);
    }
    
    function deposit(uint256 amount, uint256 secs) external {
        _deposit(amount, msg.sender, secs, block.timestamp);
    }
    
    function depositFor(uint256 amount, address account, uint256 secs) external {
    ...
    function _deposit(
        ...
        bytes32 kek_id = keccak256(abi.encodePacked(account, start_timestamp, amount, _balances[account]));
        lockedStakes[account].push(LockedStake(
            kek_id,
            start_timestamp,
            amount,
        ...
        if (lastRewardClaimTime[account] == 0) lastRewardClaimTime[account] = block.timestamp;

        emit Staked(account, amount, secs, kek_id);
    }

    function withdraw(bytes32 kek_id) external {
        _withdraw(kek_id);
    }
    ...
    function withdrawAll() public nonReentrant updateReward(msg.sender) {
        for (uint256 i = 0; i < lockedStakes[msg.sender].length; i++){ 
            ...
            if (liquidity > 0){
                ...
                emit Withdrawn(msg.sender, liquidity, lockedStakes[msg.sender][i].kek_id);
            }
        }
    }
    
    function _withdraw(bytes32 kek_id) internal nonReentrant updateReward(msg.sender) {
        ...
        if (liquidity > 0) {
            _totalSupply = _totalSupply.sub(liquidity);
            _balances[msg.sender] = _balances[msg.sender].sub(liquidity);
            delete lockedStakes[msg.sender][theArrayIndex];
            TOKEN.safeTransfer(msg.sender, liquidity);
        }

        emit Withdrawn(msg.sender, liquidity, kek_id);
    }
    ...
    event RewardAdded(uint256 reward);
    event Staked(address indexed user, uint256 amount, uint256 secs, bytes32 kek_id);
    event Withdrawn(address indexed user, uint256 amount, bytes32 kek_id);
    event RewardPaid(address indexed user, uint256 reward);
}
