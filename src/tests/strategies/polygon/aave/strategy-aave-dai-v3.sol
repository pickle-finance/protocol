...
import "../../../lib/polygon/aave/test-defi-base.sol";
...
contract StrategyAaveDaiV3Test is DSTestDefiBase {
    StrategyAaveDaiV3 strategy;
    ...
    function setUp() public {
        ...
        strategy = new StrategyAaveDaiV3(
            governance,
            strategist,
            address(controller),
            timelock
        ...
        // Set performance treasury fee to 4.5%
        strategy.setPerformanceTreasuryFee(450);
        // 0.175% goes to dev
        strategy.setWithdrawalDevFundFee(175);
        // 0.325% goes to treasury
        strategy.setWithdrawalTreasuryFee(325);
    }
    ...
    function testFail_aave_dai_v3_onlyKeeper_leverage() public {
    ...
    function testFail_aave_dai_v3_onlyKeeper_deleverage() public {
        _getERC20(want, 100e18);
        uint256 _want = IERC20(want).balanceOf(address(this));
        IERC20(want).approve(address(pickleJar), _want);
        pickleJar.deposit(_want);
    ...
    function test_aave_dai_v3_matic_accrued() public {
        ...
        uint256 maticAccrued = strategy.getMaticAccrued();

        assertEq(maticAccrued, 0);

        hevm.warp(block.timestamp + 1 days);
        hevm.roll(block.number + 6171); // Roughly number of blocks per day

        maticAccrued = strategy.getMaticAccrued();
        assertTrue(maticAccrued > 0);
    }

    function test_aave_dai_v3_aave_sync() public {
        _getERC20(want, 1000000e18);
        uint256 _want = IERC20(want).balanceOf(address(this));
        IERC20(want).approve(address(pickleJar), _want);
        pickleJar.deposit(_want);
    ...
    function test_aave_dai_v3_leverage() public {
        _getERC20(want, 100e18);
        uint256 _want = IERC20(want).balanceOf(address(this));
        IERC20(want).approve(address(pickleJar), _want);
        pickleJar.deposit(_want);
    ...
    function test_aave_dai_v3_deleverage() public {
        _getERC20(want, 100e18);
        uint256 _want = IERC20(want).balanceOf(address(this));
        IERC20(want).approve(address(pickleJar), _want);
        pickleJar.deposit(_want);
    ...
    function test_aave_dai_v3_withdrawSome() public {
        _getERC20(want, 100e18);
        uint256 _want = IERC20(want).balanceOf(address(this));
        IERC20(want).approve(address(pickleJar), _want);
        pickleJar.deposit(_want);
    ...
    function test_aave_dai_v3_withdrawAll() public {
        _getERC20(want, 100e18);
    ...
    function test_aave_dai_v3_earn_harvest_rewards() public {
        _getERC20(want, 100e18);
    ...
    function test_aave_dai_v3_functions() public {
        _getERC20(want, 100e18);
    ...
    function test_aave_dai_v3_deleverage_stepping() public {
        _getERC20(want, 100e18);
        uint256 _want = IERC20(want).balanceOf(address(this));
        IERC20(want).approve(address(pickleJar), _want);
        pickleJar.deposit(_want);
