  ...
  describe(`${strategyName}`, () => {
    ...
    const deposit = async (
      ...
      if (!depositA.eq(0))
        await token0
          .connect(user)
          .approve(pickleJar.address, ethers.constants.MaxUint256);
      if (!depositB.eq(0))
        await token1
          .connect(user)
          .approve(pickleJar.address, ethers.constants.MaxUint256);
      ...
      await pickleJar.connect(user).deposit(depositA, depositB, true);
    ...
    const depositWithEthToken1 = async (
      ...
      await pickleJar
        .connect(user)
        .deposit(depositA, 0, true, { value: depositB });
      console.log(
        "User Balance after => ",
        (await user.getBalance()).toString()
      );
    ...
    const depositWithEthToken0 = async (
      ...
      await pickleJar
        .connect(user)
        .deposit(0, depositB, true, { value: depositA });
      console.log(
        "User Balance after => ",
        (await ethers.provider.getBalance(user.address)).toString()
      );
