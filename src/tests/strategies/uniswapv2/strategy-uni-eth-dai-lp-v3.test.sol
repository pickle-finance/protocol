...
import "../../../strategies/uniswapv2/strategy-uni-eth-dai-lp-v3.sol";


contract StrategyUniEthDaiLpV3Test is DSTestDefiBase {
    ...
    StrategyUniEthDaiLpV3 strategy;
    ...
    function setUp() public {
        ...
        strategy = new StrategyUniEthDaiLpV3(
            governance,
            strategist,
            address(controller),
            timelock
    ...
    function _getWant(uint256 ethAmount, uint256 daiAmount) internal {
        ...
        IERC20(dai).approve(address(univ2), _dai);
    ...
    function test_ethdaiv3_get_earn_harvest_rewards() public {
        _getWant(10 ether, 4000 ether); // WANT w/ 10 ETH and 400 DAI in 18 decimals
        uint256 _want = IERC20(want).balanceOf(address(this));
        IERC20(want).approve(address(pickleJar), _want);
        pickleJar.deposit(_want);
