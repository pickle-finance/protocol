...
import {expect, increaseTime, getContractAt, increaseBlock} from "../../../utils/testHelper";
...
export const doTestBehaviorBase = (
  ...
  describe(`${strategyName} common behavior tests`, () => {
    ...
    it("should add and remove rewards correctly", async () => {
      ...
      const invalidToNativeRoute = [[ethOpUsdc], [native.address, notRewardToken]];
      ...
      expect(rewardsAfterAdd.filter((z) => z.toLowerCase() === reward_addr.toLowerCase()).length).to.be.eq(
        1,
        "Updating reward path results in redundance in activeRewardsTokens"
      );

      // Attempt to add wrong route
      ...
      expect(callReverted).to.be.eq(true, "Strategy accepts adding reward with wrong route");

      // Perform a harvest and ensure it is successful
      const _want: BigNumber = await want.balanceOf(alice.address);
      await want.approve(pickleJar.address, _want);
      await pickleJar.deposit(_want);
      await pickleJar.earn();
      ...
      expect(ratioAfter).to.be.gt(ratioBefore, "Harvest failed after setting new toNative route");
