...
const {setupPusd} = require("../utils/setupHelper");
const {NULL_ADDRESS} = require("../utils/constants");

const doTestPusdBehaviorBase = (strategyName, want_addr) => {
  let alice, want;
  ...
  describe(`${strategyName} common behavior tests`, () => {
    before("Setup contracts", async () => {
      ...
      [controller, strategy, pickleJar] = await setupPusd(
        strategyName,
        want,
        governance,
        strategist,
    ...
    it("Should withdraw correctly", async () => {
      ...
      await pickleJar.deposit(_want, want.address);
      ...
      await pickleJar.withdrawAll(want.address);
...
module.exports = {doTestPusdBehaviorBase};
