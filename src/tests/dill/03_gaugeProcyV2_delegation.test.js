...
let GaugeProxyV2, userSigner, populatedTx, masterChef, governanceSigner;
...
describe("Vote & Distribute", () => {
  ...
  it("delegate should vote again successfully ", async () => {
    /** here gov will vote on behalf of user */
    await GaugeProxyV2.connect(governanceSigner).voteFor(userAddr, [pickleLP, pyveCRVETH], [4000000, 6000000]);
  });
  ...
  it("delegate should not be able to vote if owner has already voted for current period", async () => {
    await expect(
      GaugeProxyV2.connect(governanceSigner).voteFor(userAddr, [pickleLP, pyveCRVETH], [400000, 600000])
  ...
  it("Delegate vote should fail for user as delegation period has expired", async () => {
    /** here gov will vote on behalf of user */
    await expect(
      GaugeProxyV2.connect(governanceSigner).voteFor(userAddr, [pickleLP, pyveCRVETH], [4000000, 6000000])
    ).to.be.revertedWith("Delegating period expired");
  });
  it("Should reset delegate successfully and again reset to indefinite", async () => {
    const gaugeProxyFromUser = GaugeProxyV2.connect(userAddr);
    populatedTx = await gaugeProxyFromUser.populateTransaction.setVotingDelegate(dillHolder, 2, false);
    await userSigner.sendTransaction(populatedTx);

    populatedTx = await gaugeProxyFromUser.populateTransaction.setVotingDelegate(dillHolder, 2, true);
    await userSigner.sendTransaction(populatedTx);
  });
});
